...
import com.agro.utils.FastDFSClient;
import com.agro.utils.FastDFSFile;
...
//            String sr = FastDFSClient.getTrackerUrl()+"/"+upload[0]+"/"+upload[1];
            String sr = "/"+upload[0]+"/"+upload[1];

            /*FastDFSClient.getTrackerUrl()+"/"+upload[0]+"/"+upload[1]*/
            return ServerResponse.success(sr);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public ServerResponse uploadImg(@RequestParam(value = "img_file") MultipartFile img_file) {

        try {
            //1. 创建图片文件对象(封装)
            //2. 调用工具类实现图片上传

            String substring = img_file.getOriginalFilename().substring(img_file.getOriginalFilename().lastIndexOf(".") + 1);

            FastDFSFile fastdfsfile = new FastDFSFile(
                    img_file.getOriginalFilename(),//原来的文件名  1234.jpg
                    img_file.getBytes(),//文件本身的字节数组
                    StringUtils.getFilenameExtension(img_file.getOriginalFilename())
            );
            String[] upload = FastDFSClient.upload(fastdfsfile);
