...
layui.define(['jquery'],function (exports) {
    ...
    var settings = {
        base_url: modPath
        , images_upload_url: '/file/oss_upload_image'//图片上传接口，可在option传入，也可在这里修改，option的值优先
        , language: 'zh_CN'//语言，可在option传入，也可在这里修改，option的值优先
        , response: {//后台返回数据格式设置
            statusName: response.statusName || 'status'//返回状态字段
        ...
        , success: function (res, succFun, failFun) {//图片上传完成回调 根据自己需要修改
            if (res[this.response.statusName] == 200) {
                console.log("res:")
                console.log(res[this.response.dataName])
